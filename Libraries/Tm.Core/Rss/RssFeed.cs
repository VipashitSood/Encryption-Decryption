...
namespace Tm.Core.Rss
{
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(TmRssDefaults.Title, title);
            Description = new XElement(TmRssDefaults.Description, description);
            Link = new XElement(TmRssDefaults.Link, link);
            LastBuildDate = new XElement(TmRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(TmRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(TmRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(TmRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(TmRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(TmRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(TmRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(TmRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(TmRssDefaults.Channel,
