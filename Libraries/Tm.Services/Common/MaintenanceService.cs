...
namespace Tm.Services.Common
{
    ...
    public partial class MaintenanceService : IMaintenanceService
    {
        ...
        public MaintenanceService(ITmFileProvider fileProvider)
        {
        ...
        {
            var path = _fileProvider.GetAbsolutePath(TmCommonDefaults.DbBackupsPath);
        ...
        public virtual IList<string> GetAllBackupFiles()
        {
            ...
            {
                throw new TmException("Backup directory not exists");
            }

            return _fileProvider.GetFiles(path, $"*.{TmCommonDefaults.DbBackupFileExtension}")
        ...
        {
            return _fileProvider.Combine(GetBackupDirectoryPath(), $"database_{DateTime.Now:yyyy-MM-dd-HH-mm-ss}_{CommonHelper.GenerateRandomDigitCode(10)}.{TmCommonDefaults.DbBackupFileExtension}");
        }
