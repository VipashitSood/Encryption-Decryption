...
namespace Tm.Services.Common
{
    ...
    /// </summary>
    public partial class TmHttpClient
    {
        #region Fields
        ...
        public TmHttpClient(AdminAreaSettings adminAreaSettings,
            HttpClient client,
            ...
            IWorkContext workContext)
        {
            ...
            client.DefaultRequestHeaders.Add(HeaderNames.UserAgent, $"nopCommerce-{TmVersion.CurrentVersion}");
        ...
        public virtual async Task<string> GetCopyrightWarningAsync()
        {
            ...
            var url = string.Format(TmCommonDefaults.TmCopyrightWarningPath,
        ...
        public virtual async Task<RssFeed> GetNewsRssAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(_workContext.WorkingLanguage);
            var url = string.Format(TmCommonDefaults.TmNewsRssPath,
                TmVersion.CurrentVersion,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        {
            //prepare URL to request
            var url = string.Format(TmCommonDefaults.TmInstallationCompletedPath,
                TmVersion.CurrentVersion,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        public virtual async Task<string> GetExtensionsCategoriesAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(_workContext.WorkingLanguage);
            var url = string.Format(TmCommonDefaults.TmExtensionsCategoriesPath, language).ToLowerInvariant();
        ...
        public virtual async Task<string> GetExtensionsVersionsAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(_workContext.WorkingLanguage);
            var url = string.Format(TmCommonDefaults.TmExtensionsVersionsPath, language).ToLowerInvariant();
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(_workContext.WorkingLanguage);
            var url = string.Format(TmCommonDefaults.TmExtensionsPath,
