...
namespace Tm.Services.Media
{
    ...
    public partial class AzurePictureService : PictureService
    {
        ...
        public AzurePictureService(ITmDataProvider dataProvider,
        ...
        protected void OneTimeInit(TmConfig config)
        {
            if (_isInitialized)
                return;
        ...
        protected virtual async Task DeletePictureThumbsAsync(Picture picture)
        {
            ...
            _staticCacheManager.RemoveByPrefix(TmMediaDefaults.ThumbsExistsPrefixCacheKey);
        }
        ...
        protected virtual async Task<bool> GeneratedThumbExistsAsync(string thumbFilePath, string thumbFileName)
        {
            try
            {
                var key = _cacheKeyService.PrepareKeyForDefaultCache(TmMediaDefaults.ThumbExistsCacheKey, thumbFileName);
