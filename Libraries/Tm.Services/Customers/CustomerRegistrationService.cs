...
namespace Tm.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            switch (request.PasswordFormat)
            {
                ...
                case PasswordFormat.Encrypted:
                    ...
                    break;
                case PasswordFormat.Hashed:
                    var saltKey = _encryptionService.CreateSaltKey(TmCustomerServicesDefaults.PasswordSaltKeySize);
        ...
        public virtual void SetEmail(Customer customer, string newEmail, bool requireValidation)
        {
            ...
            if (newEmail == null)
                throw new TmException("Email cannot be null");

            newEmail = newEmail.Trim();
            var oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new TmException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new TmException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new TmException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
            ...
            if (requireValidation)
            {
                ...
                _genericAttributeService.SaveAttribute(customer, TmCustomerDefaults.EmailRevalidationTokenAttribute, Guid.NewGuid().ToString());
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new TmException("Usernames are disabled");

            newUsername = newUsername.Trim();

            if (newUsername.Length > TmCustomerServicesDefaults.CustomerUsernameLength)
                throw new TmException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new TmException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
