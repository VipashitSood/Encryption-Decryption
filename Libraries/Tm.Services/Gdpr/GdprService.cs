...
namespace Tm.Services.Gdpr
{
    ...
    public partial class GdprService : IGdprService
    {
        ...
        public virtual IList<GdprConsent> GetAllConsents()
        {
            ...
            var gdprConsents = query.ToCachedList(_cacheKeyService.PrepareKeyForDefaultCache(TmGdprDefaults.ConsentsAllCacheKey));
        ...
        public virtual void PermanentDeleteCustomer(Customer customer)
        {
            ...
            {
                var registeredRole = _customerService.GetCustomerRoleBySystemName(TmCustomerDefaults.RegisteredRoleName);
                _customerService.RemoveCustomerRoleMapping(customer, registeredRole);
            }

            if (!_customerService.IsGuest(customer))
            {
                var guestRole = _customerService.GetCustomerRoleBySystemName(TmCustomerDefaults.GuestsRoleName);
