...
namespace Tm.Framework.Validators
{
    ...
    public abstract class BaseTmValidator<TModel> : AbstractValidator<TModel> where TModel : class
    {
        #region Ctor

        protected BaseTmValidator()
        {
            PostInitialize();
        }
        ...
        protected virtual void SetDatabaseValidationRules<TEntity>(ITmDataProvider dataProvider, params string[] filterStringPropertyNames)
        ...
        protected virtual void SetDecimalMaxValue(EntityDescriptor entityDescriptor)
        {
            ...
            foreach (var expression in maxValueExpressions)
            {
                RuleFor(expression.Expression).IsDecimal(expression.MaxValue)
                    .WithMessage(string.Format(localizationService.GetResource("Tm.Framework.Validators.MaxDecimal"), expression.MaxValue - 1));
            }
        }
