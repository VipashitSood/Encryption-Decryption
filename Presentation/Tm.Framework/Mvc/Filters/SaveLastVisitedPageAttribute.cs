    ...
    public sealed class SaveLastVisitedPageAttribute : TypeFilterAttribute
    {
        ...
        private class SaveLastVisitedPageFilter : IActionFilter
        {
            ...
            public void OnActionExecuting(ActionExecutingContext context)
            {
                ...
                var previousPageUrl = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer, TmCustomerDefaults.LastVisitedPageAttribute);

                //save new one if don't match
                if (!pageUrl.Equals(previousPageUrl, StringComparison.InvariantCultureIgnoreCase))
                    _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, TmCustomerDefaults.LastVisitedPageAttribute, pageUrl);
